-- ============================================================
-- Fair auto-assign: configurable rest, randomised tie-break,
-- rolling fairness window, outpost-diversity penalty.
-- ============================================================

-- ────────────────────────────────────────────────────────────
-- 1. camp_settings table (one row per camp)
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.camp_settings (
  camp_id     INT PRIMARY KEY REFERENCES public.camps(id) ON DELETE CASCADE,
...
-- ────────────────────────────────────────────────────────────
-- 2. RPC: get camp settings (creates default row if missing)
-- ────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.rpc_get_camp_settings(TEXT, TEXT, INT);

CREATE OR REPLACE FUNCTION public.rpc_get_camp_settings(
  p_user_id  TEXT,
  p_phone    TEXT,
  p_camp_id  INT
) RETURNS TABLE (
  out_camp_id        INT,
  out_min_rest_hours NUMERIC
)
LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  PERFORM public.assert_camp_member(p_user_id, p_phone, p_camp_id);

  INSERT INTO public.camp_settings (camp_id)
  VALUES (p_camp_id)
  ON CONFLICT (camp_id) DO NOTHING;

  RETURN QUERY
    SELECT cs.camp_id, cs.min_rest_hours
    FROM   public.camp_settings cs
    WHERE  cs.camp_id = p_camp_id;
END;
$$;

REVOKE ALL  ON FUNCTION public.rpc_get_camp_settings(TEXT, TEXT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_get_camp_settings(TEXT, TEXT, INT) TO anon;

-- ────────────────────────────────────────────────────────────
-- 3. RPC: update camp settings (commander only)
-- ────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.rpc_update_camp_settings(
  p_user_id        TEXT,
  p_phone          TEXT,
  p_camp_id        INT,
  p_min_rest_hours NUMERIC DEFAULT NULL
) RETURNS VOID
LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  ...
  INSERT INTO public.camp_settings (camp_id, min_rest_hours)
  VALUES (p_camp_id, COALESCE(p_min_rest_hours, 8.0))
  ON CONFLICT (camp_id) DO UPDATE SET
    min_rest_hours = COALESCE(p_min_rest_hours, public.camp_settings.min_rest_hours),
    updated_at     = now();
END;
$$;

REVOKE ALL  ON FUNCTION public.rpc_update_camp_settings(TEXT, TEXT, INT, NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_update_camp_settings(TEXT, TEXT, INT, NUMERIC) TO anon;

-- ────────────────────────────────────────────────────────────
-- 4. Rewrite rpc_auto_assign with all fairness improvements
-- ────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Read configurable rest from camp_settings (default 8h)
  ...
  -- 30-day lookback for rolling fairness
  ...
  WHILE v_day <= v_end_day LOOP
    ...
    LOOP
      ...
      FOR v_guard IN
        ...
        ORDER BY
          -- rolling 30-day total assignment count (fairness)
          (
            SELECT COUNT(*)
          ...
          -- same-outpost penalty (diversity)
          (
            SELECT COUNT(*)
          ...
          -- randomised tie-break (no more deterministic g.id)
          random()
      LOOP
      ...
      IF COALESCE(array_length(v_assigned, 1), 0) < v_needed THEN
        FOR v_guard IN
          ...
          ORDER BY
            -- prefer guard with largest gap from any existing shift
            (
              SELECT MIN(
                LEAST(
                  ABS(
...
IS 'Auto-assign guards with configurable rest, rolling fairness, outpost diversity, and randomised tie-breaking.';
