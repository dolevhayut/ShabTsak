...
const FAQ_DATA = [
  { q: "יש לי ביישוב 3 עמדות שונות, וכל אחת דורשת כמות שומרים אחרת. המערכת יודעת להתמודד עם זה?", a: "בהחלט. אפשר להגדיר לכל עמדה מינימום שומרים שונה, שעות פעילות שונות, ואפילו להגביל שומרים מסוימים לעמדות ספציפיות (למשל, רק יוצאי יחידות קרביות לש״ג הראשי)." },
  { q: "איך המערכת עוזרת לי למנוע ריבים של 'למה אני תמיד שומר בלילה והוא בבוקר'?", a: "האלגוריתם שלנו מחלק את הנטל בצורה הוגנת ומתחשב בהיסטוריית השיבוצים. בנוסף, יש אזור דוחות שקוף שמראה בדיוק כמה משמרות לילה/סופ״ש כל אחד עשה, כך שהכל מדיד ואין מקום לוויכוחים." },
  { q: "מה קורה אם שני מתנדבים לא מסתדרים ואני לא רוצה שישמרו יחד באותה עמדה?", a: "בדיוק בשביל זה פיתחנו את פיצ׳ר 'הגבלות שיבוץ'. בלחיצת כפתור תוכל להגדיר ששומר א׳ ושומר ב׳ לעולם לא ישובצו יחד לאותה משמרת, והאלגוריתם יתחשב בזה אוטומטית." },
  { q: "מישהו הבריז לי ברגע האחרון. אני שוב צריך להתחנן בוואטסאפ שיחליפו אותו?", a: "ממש לא. כשיש חוסר, המערכת (ועוזר ה-AI) מזהה מי פנוי, מי לא שמר הרבה זמן, ומי לא חסום באילוצים – ומאפשרת לך לפנות נקודתית לאנשים הרלוונטיים בלי להספים את כל הקבוצה." },
  { q: "נשמע אש. כמה זמן לוקח להקים את זה וכמה זה עולה?", a: "ההקמה לוקחת בדיוק 3 דקות: פותחים בסיס, מגדירים עמדות, ושולחים לינק הזמנה בוואטסאפ לכל הצוות. והכי חשוב – המערכת חינמית לחלוטין לכיתות כוננות מתוך שליחות." },
];
